...
obstacles_list = []
for i in range(4):
    obstacles_list.append(((random.randint(-20,200)),(random.randint(-20,200))))
    scorekeeper.penup()
    scorekeeper.setposition(obstacles_list[i][0],obstacles_list[i][1]-30)
    scorekeeper.pendown()
    scorekeeper.begin_fill()
    scorekeeper.circle(30)
    scorekeeper.end_fill()
    scorekeeper.penup()
scorekeeper.setposition(-250,250)
...
while running:
    ...
    for chaser in chaser_list:
        ...
        if (chaser.distance(0,0) > 250 or chaser.distance(0,0) < -250):
            chaser.left(180)
    for i in range(4):
        if (gmc.distance(obstacles_list[i][0],obstacles_list[i][1])<30):
            gmc.setposition(-250,250)
